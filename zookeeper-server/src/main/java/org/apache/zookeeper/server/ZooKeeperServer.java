...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    throws IOException, InterruptedException {
        ...
        if (zkDb == null) {
            // 创建ZKDatabase，ZKDatabase负责管理所有的会话记录、事务日志的存储和DataTree
    ...
    public synchronized void startup() {
        ...
        // 启动session追踪器
        startSessionTracker();
        // 初始化请求处理链，zookeeper请求处理方式为责任链模式的实现，会用多个请求处理器依次处理一个客户端请求
        // 在服务启动时，会将这些请求处理器串联成一个请求处理链：
        // PrepRequestProcessor -> SyncRequestProcessor -> FinalRequestProcessor
        setupRequestProcessors();

        //注册JMX，zookeeper会将服务器运行时的一些信息以JMX的方式暴露给外部，将zookeeper服务器实例注册给ServerCnxnFactory，之后zookeeper就可以对外提供服务
        registerJMX();
