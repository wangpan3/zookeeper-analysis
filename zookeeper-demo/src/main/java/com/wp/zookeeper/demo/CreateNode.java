...
//        countDownLatch.await();
    ...
    private void createNodeSync() throws Exception {
        ...
        // World 只有一个ID：anyone
        // Perms.ALL:代表权限
        zooKeeper.create("/persistent","持久节点内容".getBytes(Charset.forName("utf-8")),
                ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        String node_PERSISTENT_SEQUENTIAL = zooKeeper.create("/persistent_sequential", "持久顺序节点内容".getBytes(Charset.forName("utf-8")),
                        ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT_SEQUENTIAL);
        String node_EPERSISTENT = zooKeeper.create("/ephemeral", "临时节点内容".getBytes(Charset.forName("utf-8")), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
